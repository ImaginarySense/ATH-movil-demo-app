      ...
      $cordovaBarcodeScanner.scan().then(function(imageData) {
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            qrData.bundleId = data.bundleId;
            qrData.auth_key = data.auth_key;
            qrData.ref_id = data.ref_id;
...
.controller('LoginCtrl', function($scope, LoginService, $ionicPopup, $state, $window) {
  ...
  .controller('BoletoCtrl', function($scope, LoginService, $ionicPopup, $state, $window) {
    ...
    if(jsonUrl === "qr"){
                      document.getElementById("toPayment").value = qrData.phone;
                      document.getElementById("amountToPay").value = qrData.amount;
                    }else{
                      document.getElementById("toPayment").value = getParameterByName("phoneNumber",jsonUrl);
                      document.getElementById("amountToPay").value = getParameterByName("amount",jsonUrl);
                    }
    });
    ...
    $scope.cancelPayment = function () {
      $scope.closeModal();
              ...
              var url2 = "http://api.salaera.com/receipts"
              var params = "bundle="+ qrData.bundleId +"&auth_key="+ qrData.auth_key +"&transaction="+ qrData.ref_id+"&status=2&data="+JSON.stringify(data);
              // alert(params);
